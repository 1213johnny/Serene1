...
{
    [Route("Extensions/Report/[action]")]
    public class ReportController1 : Controller
    {
        ...
        public ReportController1(IReportRegistry reportRegistry, IRequestContext context, IDataReportExcelRenderer excelRenderer,
            IWebHostEnvironment hostEnvironment, IOptions<EnvironmentSettings> environmentSettings)
        {
        ...
        private byte[] RenderAsPdf(IReport report, string key, string opt)
        {
            ...
            //   Request.GetBaseUri().ToString();
            //externalUrl = "https://localhost:51729/";
            //var renderUrl = UriHelper.Combine(externalUrl, "Serenity.Extensions/Report/Render?" +
            var renderUrl = UriHelper.Combine(externalUrl, "Extensions/Report/Render?" +
            ...
            var converter = new HtmlToPdfConverter1();
            ...
            if (!string.IsNullOrEmpty(wkhtmlPath))
                ...
                throw new ValidationError("Can't locate wkhtmltopdf.exe (or wkhtmltopdf in Linux) " +
                    ...
                    System.IO.Path.GetDirectoryName(typeof(ReportController1).Assembly.Location) +
